...
export const shortenUrl = async (rawUrl, customAlias = null, password = null, expiresInDays = null, maxClicks = null, title = null, tags = null) => {
  ...
  if (!expiresInDays || isNaN(expiresInDays) || expiresInDays < 1 || expiresInDays > 365) {
    const err = new Error('Expiry in days is required and must be between 1 and 365.');
    err.status = 400;
    throw err;
  }
  ...
  if (!customAlias && !password) {
    const { rows: existingRows } = await pool.query(
      'SELECT * FROM urls WHERE original_url = $1 AND password_hash IS NULL LIMIT 1',
      [originalUrl]
    );
  ...
  if (shortCode) {
    if (!/^[a-zA-Z0-9-]{3,30}$/.test(shortCode)) {
      const err = new Error('Custom alias must be 3–30 alphanumeric characters or hyphens.');
      err.status = 400;
      throw err;
    }
    const { rows } = await pool.query('SELECT id FROM urls WHERE short_code = $1', [shortCode]);
    if (rows.length > 0) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const { rows } = await pool.query('SELECT id FROM urls WHERE short_code = $1', [candidate]);
      if (rows.length === 0) {
        shortCode = candidate;
        break;
      }
  ...
  const { rows: insertRows } = await pool.query(`
    INSERT INTO urls (short_code, original_url, password_hash, max_clicks, expires_at, edit_token, title, tags)
    VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP + ($5 || ' days')::interval, $6, $7, $8)
    RETURNING *
  `, [shortCode, originalUrl, passwordHash, maxClicks || null, expiresInDays, editToken, title, tags]);
...
export const resolveUrl = async (shortCode, ip = '', userAgent = '') => {
  ...
  if (!data) {
    const { rows } = await pool.query('SELECT * FROM urls WHERE short_code = $1', [shortCode]);
    if (rows.length === 0) {
...
export const verifyUrlPassword = async (shortCode, password, ip = '', userAgent = '') => {
  const { rows } = await pool.query('SELECT * FROM urls WHERE short_code = $1 AND (expires_at IS NULL OR expires_at > CURRENT_TIMESTAMP)', [shortCode]);
  ...
  await pool.query('UPDATE urls SET clicks = clicks + 1, last_visited = CURRENT_TIMESTAMP WHERE id = $1', [data.id]);

  const visitorString = `${ip}-${userAgent}`;
  const visitorHash = crypto.createHash('md5').update(visitorString).digest('hex');
  ...
  if (data.max_clicks) {
    if (rowCount > 0) {
      const { rows: countRows } = await pool.query('SELECT COUNT(*) as count FROM unique_visitors WHERE short_code = $1', [shortCode]);
...
export const getUrlInfo = async (shortCode) => {
  ...
  const { rows: uniqueRows } = await pool.query('SELECT COUNT(*) as count FROM unique_visitors WHERE short_code = $1', [shortCode]);
  const uniqueVisitors = parseInt(uniqueRows[0].count, 10) || 0;

  const durationHours = (new Date().getTime() - new Date(data.created_at).getTime()) / (1000 * 60 * 60);
...
export const editUrlDestination = async (shortCode, newUrl, editToken) => {
  ...
  await pool.query('UPDATE urls SET original_url = $1 WHERE id = $2', [originalUrl, data.id]);
...
export const checkAliasAvailability = async (alias) => {
  if (!/^[a-zA-Z0-9-]{3,30}$/.test(alias)) {
    const err = new Error('Alias must be 3–30 alphanumeric characters or hyphens.');
    err.status = 400;
    throw err;
  }
